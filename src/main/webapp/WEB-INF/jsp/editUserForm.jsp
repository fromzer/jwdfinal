...
<fmt:bundle basename="pagecontent">
    ...
    <div class="container">
        ...
        <div class="form-container">
            <form id="web-form" action="/admin?command=saveChangeUserAccount&userId=${userData.id}" method="post">
                <h3><fmt:message key="user.page.edit.title.update"/></h3>
                <fieldset>
                    <label for="login"><b><fmt:message key="user.page.edit.login"/></b></label>
                    <input placeholder="<fmt:message key="user.page.edit.login.placeholder"/>" type="text" name="login" id="login" value="${userData.login}"
                           tabindex="1" disabled required>
                </fieldset>
                <fieldset>
                ...
                <fieldset>
                    <label for="role"><fmt:message key="user.page.edit.role"/></label>
                    <select id="role" name="role">
                        <option value="${userData.role.id}" selected><fmt:message key="user.page.edit.role.select"/></option>
                        <option value="1"><fmt:message key="user.page.edit.role.admin"/></option>
                        <option value="2"><fmt:message key="user.page.edit.role.user"/></option>
                    </select>
                </fieldset>
                <fieldset>
                    <button name="submit" type="submit" id="web-form-submit" data-submit="...Sending"><fmt:message key="page.button.save"/></button>
                </fieldset>
                <p class="center"><fmt:message key="page.inscription.or"/> <a href="/admin?command=showAllUsers"><fmt:message key="page.button.back"/></a> <fmt:message key="page.inscription.toUserList"/></p>
            </form>
        </div>
    </div>
    </body>
